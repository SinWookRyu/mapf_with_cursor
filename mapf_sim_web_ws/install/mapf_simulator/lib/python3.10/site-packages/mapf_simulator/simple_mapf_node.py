...
class SimpleAgent:
    ...
    def update_position(self, dt: float, steering: Optional[np.ndarray] = None, grid_mode: bool = False, grid_cell_size: float = 1.0, collision_wait_time: float = 1.0):
        ...
        if grid_mode:
            ...
            # 목표 도달 판정: 정확히 목표 격자에 도달했거나 목표에 매우 가까울 때
            if grid_distance == 0 or (abs(current_grid_x - goal_grid_x) <= 1 and abs(current_grid_y - goal_grid_y) <= 1 and distance < goal_threshold):
                # 정확한 목표 위치로 스냅
                self.current_pos = self.goal_pos.copy()
            ...
            # 목표 위치에 도달하지 않았으면 한 칸씩 이동
            if grid_distance > 0:
                # 이동할 방향 결정 (한 번에 한 방향만 이동)
                move_x = 0
                move_y = 0
                
                # X 방향과 Y 방향 중 더 큰 차이를 가진 방향으로만 이동
                dx = abs(goal_grid_x - current_grid_x)
                dy = abs(goal_grid_y - current_grid_y)
                
                # 디버깅: 이동 방향 결정 로그 (에이전트 3,4,5만)
                if self.id >= 3:
                    print(f"Agent {self.id}: 현재({current_grid_x}, {current_grid_y}) → 목표({goal_grid_x}, {goal_grid_y}), dx={dx}, dy={dy}")
                
                if dx >= dy:  # X 방향 차이가 더 크거나 같으면 X 방향만 이동
                    if current_grid_x < goal_grid_x:
                        move_x = 1
                    elif current_grid_x > goal_grid_x:
                        move_x = -1
                    if self.id >= 3:
                        print(f"Agent {self.id}: X 방향 이동 결정 - move_x={move_x}")
                else:  # Y 방향 차이가 더 크면 Y 방향만 이동
                    if current_grid_y < goal_grid_y:
                        move_y = 1
                    elif current_grid_y > goal_grid_y:
                        move_y = -1
                    if self.id >= 3:
                        print(f"Agent {self.id}: Y 방향 이동 결정 - move_y={move_y}")
                ...
                if -10.0 <= new_x <= 10.0 and -10.0 <= new_y <= 10.0:
                    ...
                    for other_agent in self.parent_node.agents:
                        if other_agent.id != self.id and other_agent.is_moving:  # 목표에 도달한 에이전트는 제외
                            # 격자 인덱스 기반 충돌 검사
                            ...
                            if new_grid_x == other_grid_x and new_grid_y == other_grid_y:
                                ...
                                print(f"Agent {self.id}: Agent {other_agent.id}와 격자 위치 충돌 - 이동 취소")
                                break
                            
                            # 추가로 물리적 거리 기반 충돌 검사
                            dist = np.linalg.norm(temp_pos - other_agent.current_pos)
                            if dist < (self.radius + other_agent.radius + 0.1):  # 0.1 여유 추가
                                collision_detected = True
                                self.is_colliding = True
                                other_agent.is_colliding = True
                                print(f"Agent {self.id}: Agent {other_agent.id}와 물리적 충돌 예상 - 이동 취소")
                                break
                    ...
                    if not collision_detected:
                        ...
                        for alt_x, alt_y in alternative_moves:
                            ...
                            if -10.0 <= alt_new_x <= 10.0 and -10.0 <= alt_new_y <= 10.0:
                                ...
                                for other_agent in self.parent_node.agents:
                                    if other_agent.id != self.id and other_agent.is_moving:
                                        other_grid_x, other_grid_y = other_agent.get_grid_position(grid_cell_size)
                                ...
                                if not alt_collision_detected:
                                    ...
                                    print(f"Agent {self.id}: 대안 경로로 ({self.current_pos[0]:.1f}, {self.current_pos[1]:.1f})로 이동")
                                    alternative_found = True
                                    break
                        
                        if not alternative_found:
                            print(f"Agent {self.id}: 모든 대안 경로가 막혀있어 이동이 취소되었습니다.")
                else:
...
class SimpleMAPFNode(Node):
    def __init__(self):
        ...
        # 에이전트 위치 설정 구독자
        self.agent_positions_sub = self.create_subscription(String, '/agent_positions', self.on_agent_positions, 10)
    ...
    def initialize_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            # agent.direction_initialized = False  # 방향 초기화 상태 리셋 (원래 상태로 복원)
            # agent.current_direction = None  # 현재 방향 리셋 (원래 상태로 복원)
            # agent.collision_avoidance_active = False  # 충돌 회피 상태 리셋 (원래 상태로 복원)
            # agent.last_collision_position = None  # 마지막 충돌 위치 리셋 (원래 상태로 복원)
    ...
    def on_agent_positions(self, msg: String):
        """에이전트 위치 설정 업데이트"""
        try:
            data = json.loads(msg.data)
            self.get_logger().info(f'에이전트 위치 업데이트 수신: {data}')
            
            # JSON 데이터에서 positions 배열 추출
            positions = data.get('positions', [])
            if not positions:
                self.get_logger().warn('positions 배열이 비어있습니다.')
                return
            
            # 각 에이전트 위치 정보 처리
            for agent_data in positions:
                agent_id = agent_data.get('id')
                start_pos = agent_data.get('start')
                goal_pos = agent_data.get('goal')
                
                if agent_id is not None and 0 <= agent_id < len(self.agents):
                    agent = self.agents[agent_id]
                    
                    # 시작 위치 업데이트
                    if start_pos:
                        agent.start_pos = np.array([start_pos['x'], start_pos['y']], dtype=float)
                        agent.current_pos = agent.start_pos.copy()
                        self.get_logger().info(f'에이전트 {agent_id} 시작 위치를 ({start_pos["x"]:.1f}, {start_pos["y"]:.1f})로 설정했습니다.')
                    
                    # 목표 위치 업데이트
                    if goal_pos:
                        agent.goal_pos = np.array([goal_pos['x'], goal_pos['y']], dtype=float)
                        self.get_logger().info(f'에이전트 {agent_id} 목표 위치를 ({goal_pos["x"]:.1f}, {goal_pos["y"]:.1f})로 설정했습니다.')
                    
                    # 에이전트를 격자점에 스냅
                    agent.snap_to_grid(self.grid_cell_size)
                    
                    # 목표 도달 로그 상태 리셋
                    agent.goal_reached_logged = False
                    agent.is_moving = False  # 위치 변경 시 정지 상태로 설정
                    
                else:
                    self.get_logger().warn(f'유효하지 않은 에이전트 ID: {agent_id}')
                    
        except (json.JSONDecodeError, KeyError, ValueError) as e:
            self.get_logger().error(f'에이전트 위치 데이터 파싱 실패: {e}')
