...
from std_msgs.msg import ColorRGBA, Bool, Float32, Int32
import numpy as np
...
class SimpleAgent:
    ...
    def update_position(self, dt: float, steering: Optional[np.ndarray] = None, grid_mode: bool = False, grid_cell_size: float = 1.0):
        """에이전트 위치 업데이트"""
        ...
        if distance < 0.3:  # 목표 도달 (허용 오차 증가)
        ...
        if grid_mode:
            ...
            # 디버깅: 격자 위치 출력
            print(f"Agent {self.id}: 현재 격자({current_grid_x}, {current_grid_y}), 목표 격자({goal_grid_x}, {goal_grid_y})")
            ...
            if current_grid_x == goal_grid_x and current_grid_y == goal_grid_y:
                ...
                # 정확한 목표 위치로 스냅
                self.current_pos = self.goal_pos.copy()
                print(f"Agent {self.id}: 목표 도달!")
                return
            
            # 현재 격자와 목표 격자가 다르면 한 칸씩 이동
            if current_grid_x != goal_grid_x or current_grid_y != goal_grid_y:
                ...
                if -10.0 <= new_x <= 10.0 and -10.0 <= new_y <= 10.0:
                    ...
                    if not collision_detected:
                        ...
                        print(f"Agent {self.id}: 충돌로 인해 이동이 취소되었습니다.")
                else:
...
class SimpleMAPFNode(Node):
    ...
    def update_simulation(self):
        ...
        positions = [agent.current_pos.copy() for agent in self.agents]
        radii = [agent.radius for agent in self.agents]
        for i, agent in enumerate(self.agents):
            ...
            for j, other_pos in enumerate(positions):
                ...
                if dist < 1e-6:
                    continue
                # 원하는 최소 거리 (에이전트 반지름 합과 파라미터를 고려)
                desired_sep = max(self.collision_distance, radii[i] + radii[j])
                if dist < desired_sep:
            ...
            # 예측적 충돌 회피 (조정 가능한 예측 시간)
            if agent.is_moving and np.linalg.norm(agent.velocity) > 0:
                future_pos = agent.current_pos + agent.velocity * self.prediction_time
                for j, other_agent in enumerate(self.agents):
                    if i == j or not other_agent.is_moving:
                        continue
            ...
            # 위치 업데이트에 반영 (그리드 모드일 때는 충돌 회피 비활성화)
            if self.movement_mode == 1:  # 그리드 모드
                # 타이머 기반 이동 제어 (움직이는 에이전트만 이동)
                if self.grid_move_timer >= self.grid_move_interval and agent.is_moving:
                    agent.update_position(dt, None, grid_mode=True, grid_cell_size=self.grid_cell_size)
            else:  # 연속 모드
                if agent.is_moving:  # 움직이는 에이전트만 위치 업데이트
                    agent.update_position(dt, steering, grid_mode=False, grid_cell_size=self.grid_cell_size)
            # 월드 경계 체크
        ...
        # 시각화 업데이트
        self.publish_agent_poses()
        self.publish_start_goal_poses()  # 시작위치와 목표위치 시각화
        self.publish_collision_zones()
        self.publish_grid_visualization()  # 격자 시각화 추가
    ...
    def publish_agent_poses(self):
        """에이전트 위치 시각화 (항상 업데이트하여 RViz 호환성 향상)"""
    ...
    def on_start_simulation(self, msg: Bool):
        ...
        if msg.data:
            ...
            self.get_logger().info('시뮬레이션이 시작되었습니다.')
        else:
            self.simulation_started = False
            self.get_logger().info('시뮬레이션이 중지되었습니다.')
